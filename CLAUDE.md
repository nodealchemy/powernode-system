...
- **SDWAN Manager** (`monitor`) — owns SDWAN peer drift, hub reachability, BGP session health, VIP failover, and operator-initiated SDWAN CRUD. 57 intervention policies (43 operator `sdwan.*` CRUD, also written at the operator shape, + 14 sensor-routed `system.sdwan_*` / `system.federation_*` remediations gated here since HIER-P2A); 4h approval timeout. Skills bound: `sdwan_*` reconciliation executors. Seeded by `db/seeds/system_sdwan_manager_agent.rb` (2026-05-10). Operator guide: [`docs/SDWAN_MANAGER_AGENT.md`](./docs/SDWAN_MANAGER_AGENT.md).
...
1. Always check existing skill executors before writing a new orchestration. 66 already cover most fleet/SDWAN/runtime/topology workflows. See `docs/SKILL_EXECUTORS.md`.
...
3. New autonomy actions must have a `system.<action>` intervention policy declared in `System::Governance::PolicyDeclarations`, in the set of the agent that OWNS the category (`PolicyReconciler` is the ONLY writer of those rows — it creates missing rows on every boot, the first one included, and re-homes rows whose owner moved; record a move in `PolicyReconciler::FORMER_OWNERS`. A seed never writes a policy row: `spec/db/seeds/policy_single_writer_spec.rb` pins it). A sensor-routed category also declares that agent as `owner:` on its `DecisionEngine::SIGNAL_BINDINGS` entry — `sensor_owner_gating_spec` pins that the two agree. An operator-only set gets an agent twin (`PolicyDeclarations::OPERATOR_TWINS`).
...
- `docs/SKILL_EXECUTORS.md` — 66 executor reference; `docs/SKILL_EXECUTOR_CATALOG.md` is the auto-generated catalog (regenerate via `rails system:skills:generate_catalog` — never hand-edit)
...
- `docs/FLEET_SENSORS.md` — 40 fleet sensors (tick-registered) + 2 CVE sensors reference + intervention policy table (split per-agent post 2026-05-10)
...
- `governance-gaps.md` — what a `system.governance_gap` offer means, how to approve / materialise / dismiss it, and how to read a `fleet.governance_gap_stuck` escalation (HIER-P3)
