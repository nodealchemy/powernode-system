...
| Skill executors | `docs/SKILL_EXECUTORS.md` | `app/services/system/ai/skills/` (55 executor classes, 54 with `binds_to`), `db/seeds/system_skills_seed.rb` + `db/seeds/system_provisioning_skills_seed.rb` (BOTH seed `Ai::Skill` rows — check both before adding one) |
...
- **Fleet Autonomy** (`monitor`) — non-CVE fleet reconciler running every 60s. Cert rotation, drift remediation, module composition, rolling upgrades, package repository/module ops, architecture catalog mutations. 10 skills bound. 43 intervention policies (after the 2026-05-10 split moved CVE → CVE Responder, operator SDWAN CRUD → SDWAN Manager, and Disk Image → Disk Image Manager; the autonomous `system.sdwan_*` remediations remain here) — see [`docs/FLEET_SENSORS.md`](./docs/FLEET_SENSORS.md) §Intervention Policy Reference). Seeded by `db/seeds/fleet_autonomy_agent.rb`.
...
- **SDWAN Manager** (`monitor`) — owns SDWAN peer drift, hub reachability, BGP session health, VIP failover, and operator-initiated SDWAN CRUD. 41 intervention policies; 4h approval timeout. Skills bound: `sdwan_*` reconciliation executors. Seeded by `db/seeds/system_sdwan_manager_agent.rb` (2026-05-10). Operator guide: [`docs/SDWAN_MANAGER_AGENT.md`](./docs/SDWAN_MANAGER_AGENT.md).
...
1. Always check existing skill executors before writing a new orchestration. 54 already cover most fleet/SDWAN/runtime/topology workflows. See `docs/SKILL_EXECUTORS.md`.
...
- `docs/SKILL_EXECUTORS.md` — 54 executor reference; `docs/SKILL_EXECUTOR_CATALOG.md` is the auto-generated catalog (regenerate via `rails system:skills:generate_catalog` — never hand-edit)
...
- `docs/FLEET_SENSORS.md` — 33 fleet sensors (tick-registered) + 2 CVE sensors reference + intervention policy table (split per-agent post 2026-05-10)
