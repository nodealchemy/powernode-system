...
export const PlatformFormModal: React.FC<PlatformFormModalProps> = ({
  ...
  // Raised by DiskImageHistoryTab's rollback confirm, which is a nested Modal.
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditMode ? 'Edit Platform' : 'Create Platform'}
      icon={<Layers className="w-6 h-6" />}
      maxWidth={isEditMode ? '4xl' : '2xl'}
      // The disk-image history tab can raise a nested confirm dialog, which
      // registers its own Escape handler on document.
      closeOnEscape={!rollbackConfirmOpen}
    >
              ...
              {isEditMode && editPlatform && (
                <div className="border-t border-theme pt-4 mt-2">
                  <DiskImageHistoryTab
                    ...
                    onConfirmOpenChange={setRollbackConfirmOpen}
                  />
                </div>
              )}
            </div>
