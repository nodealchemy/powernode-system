...
runtime, and a fleet autonomy layer. Each autonomy tick senses fleet state,
gates every proposed action through intervention policy and approval, executes
the approved remediations, and records the decisions as learnings. It does not
rewrite its own skills: the trigger meant to evolve a skill from repeated
learnings is refused on every tick (see M8 under
[Milestones complete](#milestones-complete)).
...
- Rails models under `System::` and `Sdwan::`, services, and controllers across
...
- Database migrations under `server/db/migrate/`
- Sidekiq cron schedule entries

This list gives no file counts, because hand-kept counts drift. For current
figures, use the sources that are generated or checked against the code:

- Skill executors: [`docs/SKILL_EXECUTOR_CATALOG.md`](./docs/SKILL_EXECUTOR_CATALOG.md),
  generated by the platform's `rails system:skills:generate_catalog`
- MCP actions: the platform's generated catalog, `docs/reference/auto/mcp-tools.md`
  in the [platform repo][platform]
- Fleet sensors: [`docs/FLEET_SENSORS.md`](./docs/FLEET_SENSORS.md), whose
  roster `server/spec/docs/reference_counts_spec.rb` checks against the code

The three figures this README does state (fleet sensors, skill executors and
scheduled jobs) are checked the same way, by `server/spec/docs/readme_counts_spec.rb`.
...
- **GitHub Discussions** — [nodealchemy/powernode-platform/discussions](https://github.com/nodealchemy/powernode-platform/discussions) for questions, general inquiries, technical support, and commercial/enterprise inquiries
...
- **M7** — FleetAutonomyService (gate_action!, 8 sensors at M7 completion,
