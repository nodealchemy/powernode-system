...
if [ -z "${STAGE1_TEST_INNER:-}" ]; then
  if unshare -rm true 2>/dev/null; then
    ...
    exec unshare -rm bash -c 'mount -t tmpfs tmpfs /tmp && exec bash "$0" "$@"' "$0" "$@"
  fi
  if [ -e /tmp/fat ]; then
...
export FAKE_CLOCK="$tmproot/clock" SLEEP_LOG="$tmproot/sleeps" CURL_CODES="$tmproot/curl-codes" \
       ...
       MM_ARCHIVES_SEEN="$tmproot/mm-archives-seen"
...
run_stage1() {
  ...
  echo 1000 > "$FAKE_CLOCK"; : > "$SLEEP_LOG"; : > "$CURL_LOG"; : > "$MM_LOG"; : > "$MM_ARCHIVES_SEEN"
...
assert_match 'mirror probe: HTTP 503 .* transient' "$OUT" "transient probe codes are logged as such"
...
assert_match 'still unhealthy after 100s .*last HTTP code 503' "$OUT" "failure names the wait budget and last HTTP code"
...
rm -rf /tmp/fat; echo 1000 > "$FAKE_CLOCK"; : > "$SLEEP_LOG"; : > "$CURL_LOG"; : > "$MM_LOG"
...
OUT=$(env -i PATH="$STUBS:/usr/bin:/bin" HOME="$tmproot" REAL_DATE="$REAL_DATE" FAKE_CLOCK="$FAKE_CLOCK" \
      ...
      MM_ARCHIVES_SEEN="$MM_ARCHIVES_SEEN" STAGE1_APT_CACHE_DIR="$CACHE" STAGE1_SNAPSHOT_BASE_URL=https://snapshot-mirror.example.test/ubuntu \
