...
# proxy verifies the worker's client cert on the websecure-mtls
# entrypoint and forwards the verified subject CN (= the worker's
# node_instance_id) via X-Forwarded-Tls-Client-Cert-Info. The legacy
# X-Worker-Token header is gone — request specs must present the cert
# header instead.
