...
#
# IMP-80a353489ba4: this spec used to live at
# platform_maintenance_executor_health_check_spec.rb, against
# System::Ai::Skills::PlatformMaintenanceExecutor#health_check. That action
# moved to its own executor (PlatformHealthCheckExecutor, bound to
# platform_health_monitor instead of concierge) — every example here moved
# with it unchanged in substance, only `described_class` and the `health_check`
# helper's call shape changed (no `action:` — this executor has exactly one).
# platform_maintenance_executor_health_check_spec.rb now pins the OTHER half
# of the split: that PlatformMaintenanceExecutor rejects "health_check".
RSpec.describe System::Ai::Skills::PlatformHealthCheckExecutor do
  ...
  describe "the false citation stays corrected in its new home" do
    # The comment this guarded used to say health_check "mirrors
    # PlatformHealthController". No class of that name exists in either
    # repository. It was pointing, inexactly, at
    # Api::V1::System::Platform::HealthController — a real and SEPARATE
    # surface. The defect is the claim of authority, not the mention: a
    # comment that names the class in order to correct the record is the fix,
    # so this asserts on the CLAIM, now against this file (the action moved
    # here from platform_maintenance_executor.rb, IMP-80a353489ba4).
    let(:source) do
      File.read(
        Rails.root.join("..", "extensions", "system", "server", "app", "services",
                        "system", "ai", "skills", "platform_health_check_executor.rb")
      )
    end

    it "does not claim to mirror a class that does not exist" do
