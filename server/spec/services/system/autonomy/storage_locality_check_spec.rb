...
RSpec.describe System::Autonomy::StorageLocalityCheck do
  ...
  describe ".network_backed_storage?" do
    it "returns nil (unverified) when the adapter doesn't support list_volume_types" do
      ...
      expect(
        described_class.network_backed_storage?(provider_adapter: adapter, region_code: "pve1", storage_name: "pve1-data")
      ).to be_nil
    end
    ...
    it "returns nil when storage_name is blank" do
      ...
      expect(
        described_class.network_backed_storage?(provider_adapter: adapter, region_code: "pve1", storage_name: nil)
      ).to be_nil
    end

    it "returns false for a local (zfspool) storage pool" do
      adapter = instance_double(System::Providers::ProxmoxProvider)
      allow(adapter).to receive(:list_volume_types).with("pve1").and_return([
    ...
    it "returns true for an nfs storage pool" do
      ...
      expect(
        described_class.network_backed_storage?(provider_adapter: adapter, region_code: "pve1", storage_name: "nas1-data")
      ).to be true
    end
    ...
    it "returns nil when the named storage isn't in the returned list" do
      ...
      expect(
        described_class.network_backed_storage?(provider_adapter: adapter, region_code: "pve1", storage_name: "unknown-storage")
      ).to be_nil
    end
  ...
  describe ".violation_for" do
    it "returns nil when not network-backed" do
      expect(described_class.violation_for(storage_name: "pve1-data", network_backed: false, node: node)).to be_nil
    end

    it "returns a confirmed INV-6 descriptor when network-backed and confirmed" do
      violation = described_class.violation_for(storage_name: "nas1-data", network_backed: true, node: node, confirmed: true)
  ...
  describe ".assert_local_storage!" do
    let(:local_adapter) do
      instance_double(System::Providers::ProxmoxProvider).tap do |a|
        allow(a).to receive(:list_volume_types).and_return([ { cloud_id: "pve1-data", name: "pve1-data", plugin_type: "zfspool", shared: false } ])
      end
    end
    let(:nfs_adapter) do
      instance_double(System::Providers::ProxmoxProvider).tap do |a|
        allow(a).to receive(:list_volume_types).and_return([ { cloud_id: "nas1-data", name: "nas1-data", plugin_type: "nfs", shared: true } ])
      end
    end

    it "does not raise when strict: false, even for network-backed storage" do
      expect {
        described_class.assert_local_storage!(provider_adapter: nfs_adapter, region_code: "pve1", storage_name: "nas1-data", strict: false)
      }.not_to raise_error
    end

    it "does not raise when strict: true and storage is confirmed local" do
      expect {
        described_class.assert_local_storage!(provider_adapter: local_adapter, region_code: "pve1", storage_name: "pve1-data", strict: true)
      }.not_to raise_error
    end

    it "raises when strict: true and storage is confirmed network-backed" do
      expect {
        described_class.assert_local_storage!(provider_adapter: nfs_adapter, region_code: "pve1", storage_name: "nas1-data", strict: true)
    ...
    it "fails closed (raises) when strict: true and the answer is undetermined" do
      ...
      expect {
        described_class.assert_local_storage!(provider_adapter: unverifiable_adapter, region_code: "pve1", storage_name: "mystery", strict: true)
