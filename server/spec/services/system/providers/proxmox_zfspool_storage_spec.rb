...
#
# The storage fixtures below are the REAL `pvesh get /nodes/pve2/storage` shapes
# measured on pve2 2026-07-26, not invented ones — `local-data` genuinely lists
...
RSpec.describe System::Providers::ProxmoxProvider do
  ...
  # As reported by PVE for node pve2.
  let(:rna_storages) do
    [
  ...
  before { allow(client).to receive(:get).with("/api2/json/nodes/pve2/storage").and_return(rna_storages) }

  describe "#pve_disk_format_for" do
    it "is raw on block-backed storage, where a qcow2 file cannot exist" do
      expect(provider.send(:pve_disk_format_for, client, node: "pve2", storage: "local-data")).to eq("raw")
      expect(provider.send(:pve_disk_format_for, client, node: "pve2", storage: "local-lvm")).to eq("raw")
    end

    it "stays qcow2 on file-backed storage — PVE quirk #5, snapshots need all-qcow2" do
      expect(provider.send(:pve_disk_format_for, client, node: "pve2", storage: "local")).to eq("qcow2")
      expect(provider.send(:pve_disk_format_for, client, node: "pve2", storage: "pve1-data")).to eq("qcow2")
    end

    it "falls back to qcow2 for unknown storage so a lookup miss cannot change existing behaviour" do
      expect(provider.send(:pve_disk_format_for, client, node: "pve2", storage: "nope")).to eq("qcow2")
    end
  end

  describe "#resolve_import_storage!" do
    it "uses the boot storage when it can carry an import — the NFS/dir path is untouched" do
      got = provider.send(:resolve_import_storage!, client, node: "pve2", storage: "pve1-data")
      expect(got).to eq("pve1-data")
    end
    ...
    it "picks an import-capable storage when the boot storage cannot carry one" do
      ...
      got = provider.send(:resolve_import_storage!, client, node: "pve2", storage: "local-data")
      expect(got).to eq("local")
    end

    it "prefers node-local staging over the shared NFS export" do
      # pve1-data (shared) also carries `import` and would win a shared-first
      ...
      got = provider.send(:resolve_import_storage!, client, node: "pve2", storage: "local-data")
      expect(got).to eq("local")
      expect(got).not_to eq("pve1-data")
    end

    it "still falls back to shared storage rather than failing when nothing local can stage" do
      allow(client).to receive(:get).with("/api2/json/nodes/pve2/storage")
    ...
    it "raises rather than guessing when nothing on the node can stage an import" do
      ...
      expect {
        provider.send(:resolve_import_storage!, client, node: "pve2", storage: "local-data")
  ...
  describe "#resolve_uefi_disk_image!" do
    ...
    it "stages into the import-capable storage, not the zfspool the disks land on" do
      expect(provider).to receive(:import_uefi_disk_image!)
        .with(client, node: "pve2", storage: "local", platform: platform, filename: "ubuntu-24.04.raw")

      volid = provider.send(:resolve_uefi_disk_image!, client, params, node: "pve2", storage: "local-data")
