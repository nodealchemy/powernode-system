    ...
    module Skills
      ...
      class ExposeServicePubliclyExecutor < BaseSkillExecutor
        ...
        def perform(service_hostname:, service_protocol:, sdwan_network_id:, sdwan_hub_peer_id:,
                    backend_port:, target_peer_id: nil, target_instance_id: nil,
          ...
          # ── Step 3: ACME certificate (reuse-or-provision; soft-fail) ───
          certificate_id     = nil
          ...
          if protocol == "https"
            ...
            if cert_result[:success]
              ...
              warnings << "certificate provisioning failed: #{cert_result[:error]}"
            end
          else
            warnings << "service_protocol=http — skipping ACME certificate (no TLS)."
          end

          # ── Step 4: reverse-proxy regen (soft-fail) ────────────────────
          if certificate_id
            proxy_result = regenerate_reverse_proxy(certificate_id: certificate_id)
            if proxy_result[:success]
              steps << "reverse_proxy_regen"
            else
              warnings << "reverse proxy regen failed: #{proxy_result[:error]}"
            end
          elsif protocol == "https"
            warnings << "reverse proxy regen skipped — no certificate was provisioned."
          end
        ...
        def ensure_virtual_ip(network_id:, hostname:, target_peer_id:)
