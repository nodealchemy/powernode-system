...
module Acme
  ...
  class IngressRoutePresenter
    # `backend_url:`/`frontend_url:`/`extra_hosts:` are the env-derived values
    # the writer would otherwise read per-router/per-cert. IngressRoutesController
    # computes them ONCE per request and threads them in so a multi-cert index
    # request parses the env 3x total instead of ~12x per cert. When omitted,
    # the writer's class methods read the env themselves (single-cert callers).
    def initialize(certificate, backend_url: nil, frontend_url: nil, extra_hosts: nil)
      @cert = certificate
    ...
    def self.project(certificate, backend_url: nil, frontend_url: nil, extra_hosts: nil)
      new(certificate,
    ...
    # The 9 derived Traefik routers for this cert, straight from the writer's
