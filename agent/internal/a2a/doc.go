...
// (verifier.go): the caller mints an Ed25519-signed token from the platform
// (proving "A may invoke skill S on B"), presents it on the call, and the
// callee verifies the signature OFFLINE against the platform's advertised
// public key — no per-call platform round-trip.
//
