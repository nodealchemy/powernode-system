...
# mformat's @@offset tells mtools where the partition STARTS but nothing about
# where it ENDS — left to itself it sizes the filesystem from the remainder of
# the image file. That silently produced a FAT that overruns its own partition
# (SIZE_GB=8 → an ~8191MiB filesystem inside a 512MiB partition). df then reports
# the filesystem's own inflated numbers, so the ESP looks roomy while any write
# landing past the real end is unrecoverable. Latent until the ESP fills enough
# for the allocator to reach the phantom region — which is exactly what an A/B
# UKI pair does. Pass the partition's true sector count explicitly.
...
# Fail the build rather than ship an ESP that overruns its partition: read the
# FAT boot sector back and assert the declared volume size fits. Cheap, and it
# is the only check that catches this — files still read back correctly while
# the filesystem is merely *capable* of overrunning, so a functional smoke test
# proves nothing here.
# FAT boot sector: total-sectors is the 16-bit field at 0x13, or 0 there and the
# 32-bit field at 0x20 for volumes too large to express in 16 bits (always the
# case for a 512MiB ESP).
