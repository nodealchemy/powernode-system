...
**Recommended remediation:** `system.instance_replace` (`require_approval`). There is **no applier** — a replace is a destructive multi-step re-provision nothing on this side performs — which is why the category is listed in `RemediationValidator::NON_REMEDIATING_ACTION_CATEGORIES`.
...
- `db/seeds/fleet_autonomy_agent.rb` — **43 policies** (non-CVE / non-SDWAN / non-disk-image fleet ops, including the 7 AUTONOMOUS `system.sdwan_*` remediations Fleet Autonomy owns)
...
### Fleet Autonomy agent (43 policies)
...
| `system.instance_replace` | `require_approval` | Disaster recovery for an instance a reboot cannot recover (`instance_unrecoverable_sensor`). Separate from `system.instance_reprovision` so "reboot it" and "throw it away and build another" are separately tunable. No applier today — detection plus the operator decision |
