...
**Threshold:** Any missing, extra or mismatched module → `system.module_drift` signal. An assigned module whose served version has no `oci_digest` is `unverifiable` in `NodeInstance#module_drift` and does **not** signal: its remedy (`sync_modules`) cannot supply a digest the version record lacks. It is named in `system_drift_report` and in `drift_check` / the compliance snapshot instead, never as an unassigned `extra`.
