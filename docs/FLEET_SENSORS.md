...
1. The 32 sensors in `FleetAutonomyService::SENSORS` run in series (cheap; per-sensor work is bounded by the data it inspects).
...
- `db/seeds/fleet_autonomy_agent.rb` — **42 policies** (non-CVE / non-SDWAN / non-disk-image fleet ops, including the 7 AUTONOMOUS `system.sdwan_*` remediations Fleet Autonomy owns)
...
### Fleet Autonomy agent (42 policies)
