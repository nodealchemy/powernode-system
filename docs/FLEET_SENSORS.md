...
1. The 37 sensors in `FleetAutonomyService::SENSORS` run in series (cheap; per-sensor work is bounded by the data it inspects).
...
```mermaid
flowchart LR
    subgraph Sensors["37 fleet sensors (registered for the Fleet Autonomy tick)"]
...
**= 141 action-category policies across the twelve system-extension agents** — the
...
### Capacity Manager agent (23 policies)
