...
> Auto-generated by `rails system:skills:generate_catalog` on 2026-08-30 19:59 UTC.
...
- [`rolling_module_upgrade`](#rolling_module_upgrade) — Compute a batched rolling-upgrade plan for a NodeModule across all instances of a Template. PLAN ONLY — the plan is NOT executed: no batch advancer, health check or circuit breaker exists, so nothing advances the batches. See the class doc for the manual procedure.
...
Compute a batched rolling-upgrade plan for a NodeModule across all instances of a Template. PLAN ONLY — the plan is NOT executed: no batch advancer, health check or circuit breaker exists, so nothing advances the batches. See the class doc for the manual procedure.
...
| `batch_pct` | integer | No | Percent of fleet per batch (1-100). Sizes groups in the returned plan only — it is NOT a blast-radius control, because nothing executes the batches. |
