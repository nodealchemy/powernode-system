...
**Why the default is still `off`.** Findings from both arms go to the agent's stderr and are not delivered to the platform. That means the service's journal, or the console for the boot composer in the initramfs. An `audit`-by-default fleet would therefore spend a verification pass on every boot-composer mount for a measurement no one reads centrally. Measure node by node with this runbook until audit findings reach the platform.
