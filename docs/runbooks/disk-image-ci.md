...
| Webhook fires but `DiskImagePublication` not created | Cosign signature verification failed | Use `platform.system_recent_signals({ kind: "system.disk_image_publish_failed", limit: 20 })` to find the failure (exact kind — the verb has no prefix filter); common cause is OIDC issuer mismatch |
