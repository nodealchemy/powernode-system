...
The system extension ships **58 skill executors** at `extensions/system/server/app/services/system/ai/skills/` (the directory holds 59 `*_executor.rb` files; one is the abstract `BaseSkillExecutor`, and all 58 concrete executors declare `binds_to`). Each pairs with an `Ai::Skill` record (seeded by `system_skills_seed.rb`) that makes it discoverable via `platform.discover_skills`. Skills are bound to autonomy + chat agents via `Ai::AgentSkill`.
