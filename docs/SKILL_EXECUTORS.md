...
The 2026-05-10 agent split moved CVE work out of Fleet Autonomy into a dedicated **CVE Responder** agent; Disk Image Manager and System Topology Designer followed, bringing the system extension to **7 seed agents** (see the extension's `CLAUDE.md` for the full roster). The binding map below is representative — Topology Designer's 8 topology skills (the 5 SDWAN composition skills plus the three composer gates it took at HIER-P2F) and the federation/platform-deploy skills are enumerated in [SKILL_EXECUTOR_CATALOG.md](./SKILL_EXECUTOR_CATALOG.md):
...
- [`SKILL_EXECUTOR_CATALOG.md`](./SKILL_EXECUTOR_CATALOG.md) — auto-generated canonical list of all 58 executors with descriptor I/O
