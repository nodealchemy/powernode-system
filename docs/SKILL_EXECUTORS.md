...
The system extension ships **48 skill executors** at `extensions/system/server/app/services/system/ai/skills/` (the directory holds 49 `*_executor.rb` files; one is the abstract `BaseSkillExecutor`). Each pairs with an `Ai::Skill` record (seeded by `system_skills_seed.rb`) that makes it discoverable via `platform.discover_skills`. Skills are bound to autonomy + chat agents via `Ai::AgentSkill`.

> **For the full 48-executor list with descriptor I/O, see [SKILL_EXECUTOR_CATALOG.md](./SKILL_EXECUTOR_CATALOG.md)** — auto-generated by `cd server && bundle exec rails system:skills:generate_catalog` (run from the **parent platform** `server/` tree; the extension has no `server/lib/tasks`). That catalog is the canonical enumeration and groups executors under the 7 `descriptor()` categories — **Devops (30), Documentation (1), Federation (5), Fleet (4), Sdwan (4), Security (3), System (1)**. It covers every executor: CVE remediation orchestration, full-stack provisioning, package management, SDWAN OVN/host-bridge/IPFIX/full-topology composition, `federation_manager`, the `architecture_*` catalog skills, workload relocation, and `platform_deploy`/`platform_maintenance`/`platform_resilience`.
>
