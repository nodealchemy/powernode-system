...
| `system-governance-gap-propose` | Platform Architect | HIER-P3 — the first executor bound to a CORE canonical (`binds_to "platform_architect"`): files or updates the one improvement offer for a `system.governance_gap` signal and materialises the runtime-closable kinds through `System::Governance::GapMaterializer` (`dev.campaign_propose`, auto_approve — the offer is the gate; a skill binding / prompt refinement applies from the `trusted` tier under `dev.skill_refine` / `dev.prompt_refine`, an edge / delegation policy parks under `dev.governance_materialize`) |
