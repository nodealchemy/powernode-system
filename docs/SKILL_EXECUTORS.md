...
The system extension ships **65 skill executors** at `extensions/system/server/app/services/system/ai/skills/` (the directory holds 66 `*_executor.rb` files; one is the abstract `BaseSkillExecutor`, and all 65 concrete executors declare `binds_to`). Each pairs with an `Ai::Skill` record (seeded by `system_skills_seed.rb`) that makes it discoverable via `platform.discover_skills`. Skills are bound to autonomy + chat agents via `Ai::AgentSkill`.

> **For the full 65-executor list with descriptor I/O, see [SKILL_EXECUTOR_CATALOG.md](./SKILL_EXECUTOR_CATALOG.md)** — auto-generated by `cd server && bundle exec rails system:skills:generate_catalog` (run from the **parent platform** `server/` tree; the extension has no `server/lib/tasks`). That catalog is the canonical enumeration (re-run the rake task rather than trusting counts restated in prose) and groups executors under the 8 `descriptor()` categories — **Devops, Documentation, Federation, Fleet, Governance, Sdwan, Security, System**. The per-category tallies that used to be restated here were stale by more than one release (they summed to 54 against a 57-executor list) and are deliberately NOT repeated: read them off the generated catalog, which is the only copy a rake task keeps true. It covers every executor: CVE remediation orchestration, full-stack provisioning, package management, SDWAN OVN/host-bridge/IPFIX/full-topology composition, `federation_manager`, the `architecture_*` catalog skills, workload relocation, and `platform_deploy`/`platform_maintenance`/`platform_resilience`.
>
...
>
> **Two category taxonomies (don't confuse them):** an executor's `descriptor()[:category]` is one of the **8 executor categories** above (used to organize the catalog). The seed in `system_skills_seed.rb` maps each executor onto a platform **`Ai::Skill` enum** value (`devops`, `security`, `sre_observability`, `release_management`, `documentation`) for cross-platform skill discovery. The two taxonomies are independent — the per-executor reference below cites the `Ai::Skill` enum value the seed assigns.

## Agent → Skill Bindings (representative)

The 2026-05-10 agent split moved CVE work out of Fleet Autonomy into a dedicated **CVE Responder** agent; Disk Image Manager and System Topology Designer followed, and the GitOps Reconciler plus the four HIER-P2 operations managers (Capacity, Storage, Ingress, Supply Chain) brought the system extension to **12 seed agents** (see the extension's `CLAUDE.md` for the full roster). The binding map below is representative — Topology Designer's 8 topology skills (the 5 SDWAN composition skills plus the three composer gates it took at HIER-P2F) and the federation/platform-deploy skills are enumerated in [SKILL_EXECUTOR_CATALOG.md](./SKILL_EXECUTOR_CATALOG.md):
...
| `system-package-repository-sync` | Supply Chain Manager | Package catalog sync (`system.package_repository.sync`, auto_approve); re-bound from Fleet Autonomy by HIER-P2E — the sensor path gates the same category under this agent |
| `system-package-module-create` | Supply Chain Manager **+** System Concierge | Package-derived module creation (`system.package_module.create`, require_approval — every new package is operator-audited); re-bound from Fleet Autonomy by HIER-P2E |
| `system-package-module-refresh` | Supply Chain Manager **+** System Concierge **+** CVE Responder | Package drift refresh; nothing sensor-routed reaches it — the CVE Responder's remediation orchestration invokes it directly under the CVE lane's gate, so that binding stays (HIER-P2E) |
| `system-architecture-propose` / `-create` / `-update` / `-delete` | Supply Chain Manager | Architecture catalog (`system.architecture.*`); re-bound from Fleet Autonomy by HIER-P2E |
| `system-suggest-architectures-for-fleet` | Supply Chain Manager **+** System Concierge | Read-shape — ranks a repo's architectures by fleet coverage; re-bound from Fleet Autonomy by HIER-P2E |
...
| `system-restore-volume` | Storage Manager | Approval-gated volume restore (`system.restore_volume`); re-bound from Fleet Autonomy by HIER-P2C so the gate resolves the owning agent's row |
| `system-docker-provision` | Runtime Manager | Container runtime lifecycle (Phase 1 Docker) |
| `system-disk-image-promote` | Disk Image Manager | Promote a published publication to the platform default (`system.disk_image_publication_promote`, require_approval); thin over `Executors::DiskImage::PromotePublication` — HIER-P2F |
| `system-disk-image-rollback` | Disk Image Manager | Roll a platform back to a prior publication (`system.disk_image_publication_rollback`, require_approval); same target selection as `system_revert_disk_image` — HIER-P2F |
| `system-disk-image-retention` | Disk Image Manager | Set a platform's retention count (`system.disk_image_retention_update`, auto_approve — the row decides, not the descriptor) — HIER-P2F |
| `system-gitops-sync-repository` | GitOps Reconciler | Sync one repository now — mints the run, hands it to `Gitops::Reconciler`, applies nothing (`system.gitops_sync_repository`, auto_approve) — HIER-P2F |
| `system-gitops-apply-proposal` | GitOps Reconciler | Apply an approved drift proposal via `Gitops::ApplyService` (`system.gitops_apply_proposal`, require_approval — the SAME control the `system_gitops_apply_proposal` MCP verb parks under) — HIER-P2F |
| `system-gitops-register-repository` | GitOps Reconciler | Register a declarative source of fleet state (`system.gitops_register_repository`, require_approval) — HIER-P2F |
| `system-sdwan-federation-compose` / `system-multi-tenant-isolation` / `system-service-discovery-composer` | System Topology Designer | The three composer gates in `TOPOLOGY_DESIGNER_POLICIES` (all require_approval, chain `Topology Designer Actions`); bound via `binds_to "topology_designer"` since HIER-P2F |
| `system-expose-service-local` | Ingress Manager **+** System Concierge | Approval-gated `/svc/<slug>` publish (`system.expose_service_local`); re-bound from System Concierge alone by HIER-P2D — the Concierge keeps the operator-chat door, the gate resolves the executing agent's row |
| `system-expose-service-publicly` | Ingress Manager **+** System Concierge | Approval-gated public HTTPS exposure (`system.expose_service_publicly`, VIP → port map → ACME → Traefik); HIER-P2D |
| `system-expose-service-public-tcp` | Ingress Manager **+** System Concierge | Approval-gated public TLS-carrying TCP exposure ON (`system.expose_service_public_tcp`); the OFF arm is the ungated `system_unexpose_service_public_tcp` MCP action on the same executor; HIER-P2D |
| `system-acme-certificate-provision` | Ingress Manager **+** System Concierge | Approval-gated DNS-01 issuance (`system.acme_certificate_provision`); Fleet Autonomy dropped by HIER-P2D — no sensor routes to this executor, the `system.acme_cert_rotate` renewal lane fires `platform_maintenance` and stays Fleet Autonomy's |
| `system-replace-instance` | Capacity Manager | Disaster-recovery replace of an unrecoverable instance (`system.instance_replace`, require_approval; the `instance_unrecoverable_sensor` lane gates under this agent). Re-bound from Fleet Autonomy by HIER-P2B |
| `system-reap-instance` | Capacity Manager | The destructive half of a replace, asked as a second approval (`system.instance_reap`). Re-bound from Fleet Autonomy by HIER-P2B |
| `system-relocate-workload` | Capacity Manager | Cross-region workload relocation (`system.relocate_workload`, require_approval). Re-bound from Fleet Autonomy by HIER-P2B |
| `system-scale-project` | Capacity Manager | Provisioning-project footprint changes (add/remove replicas, add region, vertical resize plan); dispatched by `System::AdaptationGate` under `project.scale_horizontal`. Re-bound from Fleet Autonomy by HIER-P2B |
| `system-provision-full-stack` | Capacity Manager | Compute + storage + SDWAN stack from a template — the composer `scale_project` / `relocate_workload` call. Re-bound from Fleet Autonomy by HIER-P2B |
| `system-governance-gap-propose` | Platform Architect | HIER-P3 — the first executor bound to a CORE canonical (`binds_to "platform_architect"`): files or updates the one improvement offer for a `system.governance_gap` signal and materialises the runtime-closable kinds through `System::Governance::GapMaterializer` (`dev.campaign_propose`, auto_approve — the offer is the gate; a skill binding / prompt refinement applies from the `trusted` tier under `dev.skill_refine` / `dev.prompt_refine` where that trust-conditioned pair is seeded — the `Powernode Admin` account — and at EVERY tier elsewhere, through core's account-wide floor for those two categories (IMP-a51963f8717f), an edge / delegation policy parks under `dev.governance_materialize`) |
| `system-attach-storage` | Capacity Manager | Provision + attach + mount a cloud volume on a running instance — the provisioning mission's adaptive-evolution step (`Ai::Provisioning::AdaptationProposerService`, `provisioning` subdomain). Re-bound from Fleet Autonomy by HIER-P2SWEEP (driver ruling): it runs DURING PROVISIONING, so it belongs to the provisioning-step owner; the Storage Manager owns the volume DATA plane (restore, snapshot delete, assignment reconcile), not a fresh attach, and Fleet Autonomy declares no provisioning category since HIER-P2DECL |
| `system-platform-resilience` | System Concierge **+** Capacity Manager | Operator chat door (Concierge) and the capacity keeper's scale / drain arm (`System::Platform::ReplicaReconciler` resolves `system.platform.scale_out` / `scale_in` against the executing agent — HIER-P2B) |
...
1. Create `extensions/system/server/app/services/system/ai/skills/<name>_executor.rb`. Match the canonical shape:
   ...
   ```
2. Add the skill to `extensions/system/server/db/seeds/system_skills_seed.rb`. The executor's own `descriptor[:category]` is one of the 8 executor categories (Devops / Documentation / Federation / Fleet / Governance / Sdwan / Security / System); the seed separately maps it to a platform `Ai::Skill` enum value (`devops`, `security`, `sre_observability`, `release_management`, `documentation`) for cross-platform discovery. Keep the two consistent in intent. See [`SKILL_EXECUTOR_AUTHORING.md`](./SKILL_EXECUTOR_AUTHORING.md) for the full `skill_descriptor` + `binds_to` contract.
...
- [`SKILL_EXECUTOR_CATALOG.md`](./SKILL_EXECUTOR_CATALOG.md) — auto-generated canonical list of all 65 executors with descriptor I/O
