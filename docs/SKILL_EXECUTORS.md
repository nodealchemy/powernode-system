...
The system extension ships **64 skill executors** at `extensions/system/server/app/services/system/ai/skills/` (the directory holds 65 `*_executor.rb` files; one is the abstract `BaseSkillExecutor`, and all 64 concrete executors declare `binds_to`). Each pairs with an `Ai::Skill` record (seeded by `system_skills_seed.rb`) that makes it discoverable via `platform.discover_skills`. Skills are bound to autonomy + chat agents via `Ai::AgentSkill`.

> **For the full 64-executor list with descriptor I/O, see [SKILL_EXECUTOR_CATALOG.md](./SKILL_EXECUTOR_CATALOG.md)** — auto-generated by `cd server && bundle exec rails system:skills:generate_catalog` (run from the **parent platform** `server/` tree; the extension has no `server/lib/tasks`). That catalog is the canonical enumeration (re-run the rake task rather than trusting counts restated in prose) and groups executors under the 7 `descriptor()` categories — **Devops, Documentation, Federation, Fleet, Sdwan, Security, System**. The per-category tallies that used to be restated here were stale by more than one release (they summed to 54 against a 57-executor list) and are deliberately NOT repeated: read them off the generated catalog, which is the only copy a rake task keeps true. It covers every executor: CVE remediation orchestration, full-stack provisioning, package management, SDWAN OVN/host-bridge/IPFIX/full-topology composition, `federation_manager`, the `architecture_*` catalog skills, workload relocation, and `platform_deploy`/`platform_maintenance`/`platform_resilience`.
>
...
- [`SKILL_EXECUTOR_CATALOG.md`](./SKILL_EXECUTOR_CATALOG.md) — auto-generated canonical list of all 64 executors with descriptor I/O
