...
| `system_drain_instance_pool` | Stop replenishing (enforced — `replenish!` refuses a non-active pool) + terminate ready members; claimed members keep running. Reversible: `status: "active"` warms the pool back to its standing `target_size` |
...
| `system_cordon_instance` | **Cordon-only (unschedulable) mode — the reversible half of the drain.** Marks the instance unschedulable WITHOUT stopping it: a ready pool member is fenced out of the allocator (`pool_state=draining`, the one state `InstancePoolService#acquire!` never picks and the reaper never touches — so pool acquires, CI runner leases and agent-fleet missions stop landing on it), a claimed member is marked only (and fenced rather than re-admitted when its consumer returns it to a `reuse_without_reset` pool), a non-pool instance is marked only, and on a deployment replica the replica reconciler honours the marker — out of the live count (a replacement is provisioned), first scale-in victim. Records who/why/when under the instance's `cordon` config key (visible on `system_get_instance`). Requires `system.instances.control`; **approval-gated** under `system.instance_cordon` (`require_approval` by default) — a `pending: true` envelope means it is NOT yet cordoned. See [node-provisioning.md](./runbooks/node-provisioning.md) Phase 5 | operator |
| `system_uncordon_instance` | Lifts a cordon: clears the marker and hands a member this cordon fenced back to the allocator as `ready` (with a fresh ready-TTL anchor) — only while it is still `draining` AND running; refuses to re-admit a stopped member (start it first) and never writes `ready` over a member that was recycled or errored meanwhile. Gated under the SAME `system.instance_cordon` category — one operator row governs both directions | operator |
