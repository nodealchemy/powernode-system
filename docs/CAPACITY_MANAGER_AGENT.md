...
The agent ships with **25 intervention policies**:
...
| `system.abandoned_instance_reap` | `auto_approve` | sensor: `abandoned_instance_sensor` → DecisionEngine applier (no executor) | Terminates a non-pool cloud instance nobody has heard from past the abandonment window (default 7 days) instead of raising reprovision / closure / replace cards for it; placed in the instance's plane, so a protected plane parks one approval per instance, and any plane parks it when the guest was last seen running, is stopped, has a volume attached, or is the ACTIVE holder of a virtual IP (refused unconditionally, even when approved, until the address is moved off it — a failover-only standby is not a reason at all); the claim is re-checked, the guest's peer ids are pruned from any VIP's failover-only list, and attached volumes are detached at execution, in that order (IMP-10c9b9634d4e) |
